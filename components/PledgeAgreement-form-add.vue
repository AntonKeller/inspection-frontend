<template>
  ...
  >
    ...
    <v-card-title class="d-flex align-center">
      Создание новой записи о договоре залога
      <v-btn
    ...
    <v-card-subtitle title="Заполните сведения о договоре залога"/>
    ...
    <v-card-actions>
      <my-btn-submit
          text="Создать новый договор залога"
          prepend-icon="mdi-checkbox-multiple-marked-outline"
          :loading="loading"
          @click="createPledgeAgreement"
      />
...
const {create} = usePledgeAgreementsApi();

async function createPledgeAgreement() {
  await form.value.validate();
  if (!formIsValid.value) return;
  loading.value = true;
  return create(pledgeAgreement.value)
